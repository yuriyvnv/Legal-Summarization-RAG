...
class BatchSummarizer:
    ...
    def process_documents(self, chunks_dict: Dict, output_path: str) -> Dict:
        ...
        if "cuad" not in chunks_dict:
            logger.error("'cuad' key not found in input data")
            return summarized_structure

        if "cuad" not in summarized_structure:
            summarized_structure["cuad"] = {}
            
        # Collect all chunks that need processing
        chunks_to_process = []
        chunk_info = []  # Store metadata about each chunk
        
        for doc_name, chunks in chunks_dict["cuad"].items():
            if doc_name in summarized_structure.get("cuad", {}):
        ...
        for i in range(0, total_chunks, self.batch_size):
            ...
            for chunk_text, info, summary in zip(batch_chunks, batch_info, summaries):
                ...
                if doc_name not in summarized_structure["cuad"]:
                    summarized_structure["cuad"][doc_name] = []
                
                while len(summarized_structure["cuad"][doc_name]) <= info["chunk_index"]:
                    summarized_structure["cuad"][doc_name].append(None)
                
                summarized_structure["cuad"][doc_name][info["chunk_index"]] = {
...
def main():
    ...
    output_path = "/home/yperezhohin/Legal-Summarization-RAG/baseline_v2/summarized_docs_cuadV2.json"
    ...
    try:
        ...
        summarizer = BatchSummarizer(batch_size=20)
