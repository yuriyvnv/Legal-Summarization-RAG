...
def process_batch(batch: List[Dict], client: PersistentClient, embedding_function, k_values: List[int]) -> List[Dict]:
    ...
    # Using an inner progress bar for the batch (can be disabled if needed)
...
def evaluate_dataset(eval_file_path: str, k_values: List[int], batch_size: int = 5000, num_workers: int = 4) -> Dict:
    ...
    # Initialize the client and embedding function (using GPU)
    client = PersistentClient(path="/home/yperezhohin/Legal-Summarization-RAG/data/benchmarks/baseline_ours_all_mini")
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in as_completed(future_to_batch):
            ...
            try:
                ...
                # Write intermediate results to safeguard progress
...
def generate_metrics_table(results: Dict[str, Dict], k_values: List[int], output_folder: str):
    ...
    for dataset in datasets:
        for metric in metrics:
            ...
            for k in k_values:
                key = f'{metric.lower()}_score' if metric == 'F1' else metric.lower()
                value = results[dataset]['metrics'][f'k_{k}'][key]
                metrics_data[f'k={k}'].append(round(value * 100, 2))

    # Calculate average metrics over all datasets
    for metric in metrics:
        metrics_data['Dataset'].append('ALL')
        metrics_data['Metric'].append(metric.lower())
        for k in k_values:
            key = f'{metric.lower()}_score' if metric == 'F1' else metric.lower()
            values = [results[dataset]['metrics'][f'k_{k}'][key] for dataset in datasets]
...
def main():
    ...
    output_folder = "evaluation_results_all_mini_multiK"
    os.makedirs(output_folder, exist_ok=True)

    k_values = [1, 2, 4, 8]
    
    # Adjust batch size and number of workers based on your server's resources
    # Here, we set batch_size to 5000 and use all available CPU cores.
    batch_size = 8
    ...
    # Create some space for progress bars if running in a terminal
    print("\n" * 3)
