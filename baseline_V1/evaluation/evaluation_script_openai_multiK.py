...
def evaluate_dataset(eval_file_path: str, k_values: List[int], batch_size: int = 10000, num_workers: int = 24) -> Dict:
    ...
    client = PersistentClient(path="/home/yperezhohin/Legal-Summarization-RAG/data/benchmarks/baseline_ours_openai")
    embedding_function =  embedding_function_openai= embedding_functions.OpenAIEmbeddingFunction(
        api_key=os.getenv("OPENAI_API_KEY"),
        model_name="text-embedding-3-large"  # OpenAI's latest embedding model
    )
...
def main():
    ...
    output_folder = "evaluation_results_openai_multi_k"
